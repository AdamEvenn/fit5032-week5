...
const submitForm = () => {
  ...
  if (!errors.value.username && !errors.value.password
    ...
    if (formData.value.username == '34061894' && formData.value.password == 'Wyy1234.'){
      isAuthenticated.value = true
      authenticatedRole.value = 'admin'
      router.push('Adminpage')
    } else if(formData.value.username != '34061894'){
        signInWithEmailAndPassword(getAuth(), formData.value.username, formData.value.password).then((data) => {
            console.log("Login succeed data", data)
            console.log("Login succeed", getAuth().currentUser)
            isAuthenticated.value = true
            authenticatedRole.value = 'user'
